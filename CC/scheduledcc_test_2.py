...
if len(sys.argv) == 2:
    print('missing argument: result folder')
...
def plot_final_graph(addition="", third=True, ylim_loss=4, ylim_error=1):
    history_no_clipping = pickle.load(
        open(addition + 'trainHistoryDict', "rb"))
    history_05 = pickle.load(open(addition + 'trainHistoryDict_clip_05', "rb"))
    if third:
        history_1 = pickle.load(
            open(addition + 'trainHistoryDict_clip_1', "rb"))
    fig, axs = plt.subplots(1, 2, figsize=(15, 6))
    axs[0].grid(True)
    axs[1].grid(True)
    max_len = len(history_no_clipping['val_loss'])
    axs[0].plot(history_no_clipping['val_loss'][4:max_len:5],
                label='no clipping')
    ...
    axs[1].plot(1 - np.array(history_no_clipping['val_acc'][4:max_len:5]),
                label='no clipping')
...
"""### model (according to "CircConv" article)"""
...
save_dir = "/NEW_HDD/HDD_3TB/" + sys.argv[2]
...
history = functions.run_training(model, model_type, x_train, y_train, x_test,
                                 y_test,
                                 prefix + 'trainHistoryDict',
                                 steps_per_epoch=100, batch_size=100,
                                 epochs=1)
...
callbacks = functions.standard_callbacks(model_type) + [
    functions.Clipping(0.5)]
history = functions.run_training(model, model_type, x_train, y_train, x_test,
                                 y_test,
                                 prefix + 'trainHistoryDict_clip_05',
...
history = functions.run_training(model, model_type, x_train, y_train, x_test,
                                 y_test,
                                 prefix + 'trainHistoryDict_clip_1',
                                 steps_per_epoch=100, epochs=1,
                                 batch_size=100,
                                 callbacks=callbacks)
